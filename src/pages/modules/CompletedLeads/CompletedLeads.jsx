...
//     const data = [
//         { leadId: "L00D1", customerName: "John Doe", serviceType: "Installation", productModel: "X120", orderDate: "2025-10-01", status: "Completed" },
//         { leadId: "L00D2", customerName: "Jane Smith", serviceType: "Repair", productModel: "A250", orderDate: "2025-10-05", status: "Completed" },
//         { leadId: "L00D3", customerName: "Robert Brown", serviceType: "Maintenance", productModel: "M500", orderDate: "2025-10-10", status: "Completed" },
//         { leadId: "L00D4", customerName: "John Doe", serviceType: "Installation", productModel: "X120", orderDate: "2025-10-01", status: "Completed" },
//         { leadId: "L00D5", customerName: "Jane Smith", serviceType: "Repair", productModel: "A250", orderDate: "2025-10-05", status: "Completed" },
...
//         { leadId: "L00D8", customerName: "Robert Brown", serviceType: "Maintenance", productModel: "M500", orderDate: "2025-10-10", status: "Completed" },
...
//                                 <th className="p-3 font-poppins font-medium text-[18px] text-black sm:text-gray-900">S.No</th>
//                                 <th className="p-3 font-poppins font-medium text-[18px] text-black sm:text-gray-900">Lead ID</th>
//                                 <th className="p-3 font-poppins font-medium text-[18px] text-black sm:text-gray-900">Customer Name</th>
//                                 <th className="p-3 font-poppins font-medium text-[18px] text-black sm:text-gray-900">Service Type</th>
//                                 <th className="p-3 font-poppins font-medium text-[18px] text-black sm:text-gray-900">Product Model</th>
//                                 <th className="p-3 font-poppins font-medium text-[18px] text-black sm:text-gray-900">Order Date</th>
//                                 <th className="p-3 font-poppins font-medium text-[18px] text-black sm:text-gray-900">Status</th>
//                                 <th className="p-3 font-poppins font-medium text-[18px] text-black sm:text-gray-900">Action</th>
//                             </tr>
//                         </thead>
...
//                                     <tr
//                                         key={item.leadId}
//                                         className="bg-white sm:table-row block mb-4 sm:mb-0 border-b border-[#CACACA] sm:border-b sm:border-[#CACACA] rounded-lg sm:rounded-none shadow-sm sm:shadow-none"
//                                     >
...
//                                             {item.status}
//                                         </td>
//                                         <td className="p-2 sm:p-3 block sm:table-cell text-center before:content-['Action:'] sm:before:content-none">
//                                             <div className="flex justify-center items-center">
//                                                 <GoEye
//                                                     className="text-blue-600 w-5 h-5 cursor-pointer hover:scale-110 transition"
//                                                     onClick={() =>
//                                                         navigate("/completed-leads/completed-leads-details/", { state: { lead: item } })
//                                                     }
//                                                 />
//                                             </div>
//                                         </td>
    ...
    const fetchCompletedLeads = async () => {
      try {
        ...
        const res = await fetch(
          "https://ro-service-engineer-be.onrender.com/api/engineer/lead?status=completed",
          {
            method: "GET",
            headers: {
        ...
        console.log("✅ Completed Leads Response:", result);


        if (result.success && Array.isArray(result.data)) {
          const completedLeads = result.data.filter(
            (lead) =>
              lead.status?.toLowerCase() === "completed" ||
              lead.status?.toLowerCase() === "complete"
          );


          const formattedLeads = completedLeads.map((lead) => ({
            _id: lead._id,
            leadId: lead.id || lead._id,
            customerName: lead.customer?.name || "N/A",
            serviceType: lead.serviceType || "N/A",
            productModel: lead.service?.name || "N/A",
            orderDate: lead.createdAt,
            status: lead.status,
          }));
        ...
        setError("Failed to load completed leads. Please check your token or permissions.");
      } finally {
        setLoading(false);
      }
    };
            ...
            <table className="w-full border border-gray-400 text-left">
              ...
              <tbody>
                {displayedData.map((item, index) => (
                  <tr
                    key={item._id}
                    className="bg-white border-b border-gray-200 sm:table-row block sm:rounded-none rounded-lg sm:p-0 p-4 sm:mb-0 mb-4 shadow-sm sm:shadow-none"
                  >
                    <td className="p-2 before:content-['S.No:'] sm:before:content-none before:font-semibold before:mr-2 block sm:table-cell">
                      {startIndex + index + 1}
                    </td>
                    <td className="p-2 before:content-['Lead ID:'] sm:before:content-none before:font-semibold before:mr-2 block sm:table-cell">
                      {item.leadId}
                    </td>
                    <td className="p-2 before:content-['Customer:'] sm:before:content-none before:font-semibold before:mr-2 block sm:table-cell">
                      {item.customerName}
                    </td>
                    <td className="p-2 before:content-['Service:'] sm:before:content-none before:font-semibold before:mr-2 block sm:table-cell">
                      {item.serviceType}
                    </td>
                    <td className="p-2 before:content-['Product:'] sm:before:content-none before:font-semibold before:mr-2 block sm:table-cell">
                      {item.productModel}
                    </td>
                    <td className="p-2 before:content-['Order Date:'] sm:before:content-none before:font-semibold before:mr-2 block sm:table-cell">
                      {new Date(item.orderDate).toLocaleDateString()}
                    </td>
                    <td
                      className={`p-2 ${getStatusColor(
                        item.status
                      )} before:content-['Status:'] sm:before:content-none before:font-semibold before:mr-2 block sm:table-cell`}
                    >
                      {item.status}
                    </td>
                    <td className="p-2 before:content-['Action:'] sm:before:content-none before:font-semibold before:mr-2 sm:table-cell text-center align-middle">
                      <div className="flex justify-center items-center">
                        <GoEye
                          className="text-blue-600 w-5 h-5 cursor-pointer hover:scale-110 transition"
                          onClick={() =>
                            navigate("/completed-leads/completed-leads-details/", {
                              state: { lead: item },
                            })
                          }
                        />
