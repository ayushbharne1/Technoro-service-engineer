...
const SignUp = () => {
  ...
  const handleSendOTP = async () => {
    if (formData.mobileNumber.length !== 10)
      return setLocalErrors({ mobileNumber: "Enter valid 10-digit number" });
    ...
    if (result.meta.requestStatus === "fulfilled") {
      toast.success("OTP sent successfully: " + result.payload.otp, {
        autoClose: 4000,
      });
      setOtpSent(true);
    }
  ...
  const renderProgressBar = () => (
    <div className="w-full max-w-2xl mb-8">
      ...
      <div className="flex justify-between items-center mb-2">
        {[1, 2, 3, 4, 5].map((s) => (
          <div key={s} className="flex flex-col items-center">
            <div
              className={`w-8 h-8 md:w-10 md:h-10 rounded-full flex items-center justify-center text-white font-semibold transition-all ${
      ...
      <div className="w-full h-2 bg-gray-200 rounded-full">
        <div
          className="h-full bg-[#7EC1B1] rounded-full transition-all duration-300"
    ...
    <div className="w-full max-w-2xl space-y-5">
      ...
      <div className="flex flex-col gap-2">
        ...
        {localErrors.fullName && (
          <p className="text-red-500 text-sm">{localErrors.fullName}</p>
        )}
      </div>
      ...
      <div className="flex flex-col gap-2">
        ...
        <div className="flex gap-2">
          ...
          {!otpVerified && (
            <button
              ...
              className="px-4 md:px-6 h-12 md:h-14 bg-[#7EC1B1] text-white rounded-lg whitespace-nowrap min-w-[100px] flex items-center justify-center"
            >
        ...
        {localErrors.mobileNumber && (
          <p className="text-red-500 text-sm">{localErrors.mobileNumber}</p>
        )}
      </div>
        ...
        <div className="flex flex-col gap-2">
          ...
          <div className="flex gap-2">
            ...
            <button
              ...
              disabled={isOtpLoading}
              className="px-4 md:px-6 h-12 md:h-14 bg-green-500 text-white rounded-lg min-w-[100px] flex items-center justify-center"
            >
              {isOtpLoading ? (
                <ImSpinner2 className="animate-spin" />
              ) : (
          ...
          {localErrors.otp && (
            <p className="text-red-500 text-sm">{localErrors.otp}</p>
          )}
        </div>
      )}
    ...
    <div className="w-full max-w-2xl space-y-5">
      ...
      <div className="flex flex-col gap-2">
        ...
        <div className="relative">
          <input
            ...
            maxLength="6"
            className="h-12 md:h-14 border border-gray-300 px-4 rounded-lg w-full focus:outline-none focus:border-[#7EC1B1]"
            value={formData.pincode}
            onChange={(e) => handlePincodeChange(e.target.value)}
          />
          {pincodeLoading && (
            <div className="absolute right-3 top-1/2 -translate-y-1/2">
              <ImSpinner2 className="animate-spin text-[#7EC1B1]" />
            </div>
          )}
        </div>
      </div>
      <div className="flex flex-col gap-2">
        ...
        <input
          type="text"
          placeholder="City"
          className="h-12 md:h-14 border border-gray-300 px-4 rounded-lg bg-gray-50"
          value={formData.city}
          readOnly
        />
      </div>
    ...
    <div className="w-full max-w-2xl space-y-5">
      ...
      <div className="flex flex-col gap-2">
        ...
        <input
          ...
          maxLength="10"
          className="h-12 md:h-14 border border-gray-300 px-4 rounded-lg uppercase focus:outline-none focus:border-[#7EC1B1]"
  ...
  const renderStep5 = () => (
    <div className="w-full max-w-2xl space-y-5">
      ...
      {localErrors.termsAccepted && (
        <p className="text-red-500 text-sm">{localErrors.termsAccepted}</p>
      )}
    </div>
  );
        ...
        <div className="flex flex-col items-center">
          ...
          <div className="w-full max-w-xs flex gap-4">
            {step > 1 && (
              <button
                onClick={handleBack}
                className="flex-1 h-12 md:h-14 border-2 border-[#7EC1B1] text-[#7EC1B1] rounded-lg font-semibold hover:bg-teal-50"
              >
                Back
              </button>
            )}
